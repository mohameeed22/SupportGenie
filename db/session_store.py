...
def record_event(
    ...
    *,
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (user_id, event_type, question, answer, json.dumps(details) if details else None, _now()),
    )
...
def get_all_user_ids() -> list[int]:
    rows = _ensure_connection().execute("SELECT user_id FROM users ORDER BY last_seen_at DESC").fetchall()
